        ...
        <div className="max-w-4xl mx-auto h-full flex flex-col animate-in slide-in-from-right-4 duration-300">
             ...
             <div className="flex items-center justify-between mb-3 pt-1">
                 <div className="flex items-center gap-3">
                     <button onClick={onCancel} className="text-slate-400 hover:text-white flex items-center space-x-1 text-sm transition-colors shrink-0">
                 ...
                 <div className={cn(
                     "text-[11px] font-mono px-2.5 py-1 rounded-md border shrink-0",
             ...
             <div className="flex gap-1.5 mb-3 items-center overflow-x-auto">
                 ...
                 {!showSaveInput ? (
                     <button
                         ...
                         className={cn(
                             "flex items-center gap-1 px-2 py-1.5 rounded-lg border text-[11px] transition-all shrink-0 ml-auto",
                             deployedCount > 0 
                     ...
                     >
                         <Save className="w-3 h-3" /> 存阵
                     </button>
                 ) : (
                     <div className="flex items-center gap-1.5 ml-auto shrink-0 animate-in fade-in duration-150">
                         <input
                             type="text"
                             value={newPresetName}
                             ...
                             placeholder="阵型名称..."
                             autoFocus
                             className="w-28 bg-black/40 border border-white/15 rounded-md px-2 py-1 text-[11px] text-slate-200 placeholder-slate-600 outline-none focus:border-cyan-500/50 font-mono"
                         />
                         <button onClick={handleSavePreset} className="px-2 py-1 rounded-md bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 text-[11px] hover:bg-cyan-500/30 transition-all">保存</button>
                         <button onClick={() => { setShowSaveInput(false); setNewPresetName(''); }} className="px-1.5 py-1 rounded text-slate-500 hover:text-slate-300 text-[11px]">取消</button>
                     </div>
                 )}
                 ...
                 <button
                     onClick={clearAll}
                     className={cn(
                         "flex items-center gap-1 px-2 py-1.5 rounded-lg border text-[11px] transition-all shrink-0",
                         deployedCount > 0 
                       ...
                       <div className="relative z-10 flex flex-col gap-2 h-full">
                           {rows.map(row => (
                               <div key={row} className="flex gap-2 flex-1 items-stretch">
                                   <div className="w-8 flex items-center justify-center text-[9px] font-mono text-slate-700 uppercase tracking-wider shrink-0 select-none">
                                   ...
                                   {cols.map(col => {
                                       ...
                                       return (
                                           ...
                                           >
                                               {heroId && hero && tpl ? (
                                                   <>
                                                       <HeroIcon icon={tpl.icon} name={tpl.name} className="text-sm text-slate-200 leading-none" />
                                                       <span className={cn("text-[8px] font-mono px-1 py-0.5 rounded", cfg.tagColor, "bg-black/40")}>{cfg.tag}</span>
                                                   ...
                                                   <>
                                                       ...
                                                       <span className={cn("text-[8px] font-mono", cfg.tagColor)}>{cfg.desc}</span>
                                                   </>
                                               )}
                                           </button>
                                       );
