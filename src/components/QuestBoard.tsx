...
export default function QuestBoard({ isOpen, onClose }: QuestBoardProps) {
    ...
    const questData = useMemo(() => {
        ...
        return { quests, completedIds };
    }, [activeTab, questState]);

    const stats = useMemo(() => {
        const dailyCompleted = questState.completedDailyIds?.length || 0;
        const dailyTotal = questState.activeDailyIds?.length || 0;
        const weeklyCompleted = questState.completedWeeklyIds?.length || 0;
        const weeklyTotal = questState.activeWeeklyIds?.length || 0;
        
        return { dailyCompleted, dailyTotal, weeklyCompleted, weeklyTotal };
    }, [questState]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-3 sm:p-4">
            <div className="bg-[#0d0f12]/95 border border-white/10 rounded-xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl relative overflow-hidden">
                ...
                {/* Quest List */}
                <div className="relative z-10 flex-1 overflow-y-auto px-4 sm:px-6 pb-4 custom-scrollbar space-y-3">
                                        ...
                                        <div className="flex justify-end">
                                            {canAccept && (
                                                <button
                                                    ...
                                                    className="px-4 py-1.5 bg-cyan-600/20 hover:bg-cyan-600/30 text-cyan-300 text-xs font-bold rounded-lg border border-cyan-500/30 transition-all"
                                                >
                                                    接取
                                                </button>
                                            )}
                                            ...
                                            {!canAccept && !canTurnIn && !isCompleted && (
                                                <span className="px-4 py-1.5 text-xs text-slate-600 font-bold">
                                                    进行中...
                                                </span>
                                            )}
