...
export default function BarracksPanel() {
    ...
    const baseBingxiangCost = 2; // per 1 troop
             ...
             <div className="w-1/3 bg-black/40 border border-white/5 rounded-xl flex flex-col p-4 shadow-xl">
                  <div className="border-b border-white/5 pb-4 mb-4 flex items-center justify-between">
                      <div className="flex items-center space-x-2 text-slate-300">
                          ...
                          <h2 className="font-serif text-lg tracking-widest uppercase">将领点兵</h2>
                      </div>
                      <span className="text-xs font-mono text-slate-500">{heroes.length} 英雄</span>
                  </div>
                           ...
                           return (
                               ...
                               >
                                   <div className="flex items-center space-x-3">
                                       <div className={cn("w-10 h-10 rounded bg-white/5 flex items-center justify-center font-serif text-lg font-bold border overflow-hidden", isSelected ? "border-emerald-500 text-emerald-400" : "border-white/10 text-slate-300 group-hover:text-emerald-300")}>
                                           <HeroIcon icon={t.icon} name={t.name} className="w-full h-full flex items-center justify-center" />
                                       </div>
                                       <div>
                                           <div className={cn("font-bold text-sm", isSelected ? "text-emerald-100" : "text-slate-200")}>{t.name}</div>
                                           <div className="text-[10px] text-slate-500 font-mono">LVL {hero.level}</div>
                                       </div>
                                   </div>
                                   <div className="text-right">
                                        <div className="text-xs font-mono text-slate-400">{hero.troops} / {hero.level * 100}</div>
                                        <div className="text-[10px] text-slate-600">兵力</div>
                                        <div className="text-[9px] font-mono text-amber-500/70 mt-0.5">统{t.attributes.command}</div>
                                   </div>
                               </button>
                           )
                       })}
                      ...
                      <div className="relative z-10 flex flex-col h-full animate-in slide-in-from-bottom-4 duration-300">
                           ...
                           <div className="grid grid-cols-2 gap-6 mb-8">
                               <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                                    ...
                                    <div className="text-2xl font-mono text-slate-200">
                                         {activeHero.troops} <span className="text-slate-500 text-lg">/ {maxTroops}</span>
                                    </div>
                               ...
                               <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                                    <div className="text-[10px] tracking-widest uppercase text-slate-500 mb-1">统帅 / 伤兵率</div>
                                    <div className="text-2xl font-mono text-slate-200">
                                    ...
                                    <div className="text-xs text-amber-400/70 mt-2">
                                        ...
                                        {(activeHero.wounded || 0) > 0 && (
                                            <span className="ml-2 text-orange-400">· 待救治 {activeHero.wounded}</span>
                                        )}
                                    </div>
                               </div>
                           </div>
                           ...
                           {availableCapacity > 0 ? (
                               <div className="bg-black/60 border border-emerald-500/20 p-6 rounded-xl relative mt-auto">
                                    ...
                                    {recruitAmount > 0 && (
                                        <div className="flex items-center justify-between mb-8 bg-black/40 p-4 rounded border border-white/5">
                                            <div className="text-xs text-slate-400 font-mono uppercase tracking-widest">所需军资</div>
                                            <div className="flex items-center space-x-6">
                                                <div className="flex items-center space-x-2">
                                                    <span className="text-emerald-200 text-sm font-mono">{costFood}</span>
                                                    <span className="text-[10px] text-slate-500">粮草</span>
                                                </div>
                                                <div className="flex items-center space-x-2">
                                                    <span className="text-amber-200 text-sm font-mono">{costBingxiang}</span>
                                                    <span className="text-[10px] text-slate-500">兵饷</span>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <button 
                                        ...
                                        className={cn(
                                            "w-full py-4 rounded-lg font-bold tracking-widest uppercase transition-all shadow-xl text-sm flex items-center justify-center space-x-2",
